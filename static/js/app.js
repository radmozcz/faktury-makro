...
function skryjNepovoleneMenu() {
  ...
  const menuPrava = {
    ...
    "vystavene":       "faktury_zobrazit",
    "radek":           "radek_sekce",
    "ai-asistent":     "ai_asistent",
  };
...
function navigateTo(page) {
  ...
  const pages = {
    ...
    statistiky: () => App.userRole === "admin" || maPravo("statistiky") ? renderStatistiky() : navigateTo("dashboard"),
    kalkulace:  () => App.userRole === "admin" || maPravo("kalkulace") ? renderKalkulace() : navigateTo("dashboard"),
    "ai-asistent": () => App.userRole === "admin" || maPravo("ai_asistent") ? renderAiAsistent() : navigateTo("dashboard"),
    nastaveni:  () => App.userRole === "admin" || maPravo("nastaveni") ? renderNastaveni() : navigateTo("dashboard"),
       ...
       ${data.faktury.map(f => `
            <tr class="faktura-row" data-id="${f.id}" style="${f.duplicita_id ? 'background:#fff7ed;border-left:3px solid #f59e0b' : f.ma_duplikat ? 'background:#f0fdf4;border-left:3px solid #22c55e' : f.stav==='ke_zpracovani' ? 'background:#fffbeb' : ''}">
              ...
              <td onclick="event.stopPropagation()" style="white-space:nowrap">
                ${f.id ? `<button class="btn btn-secondary btn-sm" onclick="event.stopPropagation();otevritSoubor(${f.id})" title="Zobrazit originál" style="padding:.2rem .4rem">📎</button>` : ""}
...
function vydajeHromadneNahrat(typ = "provozni") {
  ...
  if (jeSoukrome) {
    openModal(`📦 Hromadné nahrání — ${nazev}`, `
      <div class="form-group">
        <label class="form-label">Lokace (volitelně — jinak z faktury)</label>
        <select id="hromadneVolba" class="form-control">
          <option value="">— automaticky z faktury —</option>
    ...
    openModal(`📦 Hromadné nahrání — ${nazev}`, `
      <p style="color:var(--txt2);font-size:.9rem">Firma se přiřadí automaticky podle IČO odběratele na faktuře.</p>
      <div style="display:flex;gap:.5rem;justify-content:flex-end;margin-top:1rem">
        <button class="btn btn-secondary" onclick="closeModal()">Zrušit</button>
        <button class="btn btn-primary" onclick="
          closeModal();
          const inp = document.createElement('input');
          inp.type='file'; inp.accept='.pdf,.png,.jpg,.jpeg'; inp.multiple=true;
          inp.onchange=()=>{ if(inp.files.length) _vydajeHromadneZpracovat(inp.files,'provozni',''); };
          inp.click();">Vybrat soubory →</button>
      </div>`);
  }
}
...
async function openSkupinaDetail(alias) {
  ...
  // Seskupit podle zbozi_id_orig
  const skupiny = {};
  data.nakupy.forEach(n => {
    const kid = n.zbozi_id_orig || n.zbozi_id;
    const klic = n.nazev_canonical || n.nazev || "?";
    if (!skupiny[kid]) skupiny[kid] = { nazev: klic, zbozi_id: kid, nakupy: [] };
    skupiny[kid].nakupy.push(n);
  });

  // Tagy položek v záhlaví
  const tagy = Object.values(skupiny).map(s =>
    `<span style="display:inline-flex;align-items:center;gap:.3rem;background:var(--green-pale);border-radius:99px;padding:.2rem .8rem;margin:.2rem;font-size:.85rem;cursor:pointer"
       onclick="openZboziDetail(${s.zbozi_id}, '${escHtml(s.nazev).replace(/'/g,"\\'")}')">
       ${escHtml(s.nazev)} <span style="color:var(--txt2);font-size:.75rem">✎</span>
     </span>`
  ).join("");

  // Soupis nákupů
  const vsechnyNakupy = data.nakupy.map(n => `
    <tr>
  ...
  const body = `
    <div style="margin-bottom:.5rem">
      <span style="font-size:.82rem;color:var(--txt2)">Položky ve skupině (klikni pro správu aliasu):</span><br>
      ${tagy}
    </div>
    <hr style="margin:.7rem 0;border-color:var(--border)">
    <div style="font-size:.82rem;color:var(--txt2);margin-bottom:.5rem">${data.nakupy.length} nákupů celkem</div>
    <div class="table-wrap">
      <table>
        ...
        <tbody>${vsechnyNakupy || "<tr><td colspan='8' style='text-align:center;color:var(--txt2)'>Žádné nákupy</td></tr>"}</tbody>
      </table>
    </div>`;
...
async function renderNastaveni() {
  ...
  const SEKCE = [
    ...
    { skupina: "AI asistent" },
    { klic: "ai_asistent",              label: "zobrazit" },
    { skupina: "Výplaty" },
